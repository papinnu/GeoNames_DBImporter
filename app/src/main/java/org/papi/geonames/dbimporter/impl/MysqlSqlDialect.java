...
public class MysqlSqlDialect extends AbstractSqlDialect {
    ...
    private static final String CREATE_CITY = "CREATE TABLE %1$s (ID INT NOT NULL AUTO_INCREMENT, stateId INT NOT NULL, countyId INT NOT NULL, %2$s INT, %3$s VARCHAR(20) NOT NULL, %4$s VARCHAR(100) NOT NULL, PRIMARY KEY (ID), UNIQUE (stateId, countyId, %3$s));";
