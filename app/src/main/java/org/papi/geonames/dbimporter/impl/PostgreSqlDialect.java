...
public class PostgreSqlDialect extends AbstractSqlDialect {
    ...
    private static final String CREATE_CITY = "CREATE TABLE %1$s (ID SERIAL PRIMARY KEY, stateId INT NOT NULL, countryId INT NOT NULL, %2$s INT, %3$s VARCHAR(20) NOT NULL, %4$s VARCHAR(100) NOT NULL, CONSTRAINT constraint_mapping UNIQUE (stateId, countryId, %3$s));";
